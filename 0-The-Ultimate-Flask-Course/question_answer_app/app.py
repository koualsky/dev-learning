...
def index():
    ...
    return render_template('home.html', questions=questions_results, user=user)
...
def unanswered():
    ...
    questions_cur = db.execute(
        #                                                            extra example of JOIN. previously this line was without this all join method.
        # 'select id, question_text, asked_by_id from questions      join users on users.id = questions.asked_by_id     where answer_text is null and expert_id = ?',
        # 'select id, question_text, asked_by_id from questions join users on users.id = questions.asked_by_id where answer_text is null and expert_id = ?',
        'select questions.id, questions.question_text, users.name from questions join users on users.id = questions.asked_by_id where questions.answer_text is null and questions.expert_id = ?',
        [user['id']]
    )
